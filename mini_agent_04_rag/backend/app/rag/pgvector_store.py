...
from app.schemas import RagChunk, SearchItem
...
def indexed_documents(metadata_filter: dict | None = None) -> list[SearchItem]:
    metadata_filter = metadata_filter or {}
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute(
            """
...
def source_documents(source: str) -> list[SearchItem]:
