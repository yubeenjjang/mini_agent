"""이미지 형식과 크기를 검증하고 Media Provider에 분석을 요청합니다.

Stage 01 이미지 분석 Endpoint에서 사용합니다.
"""

from app.core.config import settings
from app.providers.openai_media import analyze_image as analyze_with_provider
...
def _matches_signature(content_type: str, content: bytes) -> bool:
    checks = {"image/jpeg": content.startswith(b"\xff\xd8\xff"), "image/png": content.startswith(b"\x89PNG\r\n\x1a\n"), "image/gif": content.startswith((b"GIF87a", b"GIF89a")), "image/webp": content.startswith(b"RIFF") and content[8:12] == b"WEBP"}
...
def analyze_image(content_type: str, content: bytes, question: str) -> TravelImageAnalysis:
    ...
    return analyze_with_provider(content_type, content, question)
