from app.tools.weather import open_meteo
...
def test_current_weather_is_normalized(monkeypatch) -> None:
    ...
    monkeypatch.setattr(open_meteo.httpx, "get", lambda *args, **kwargs: FakeResponse(next(responses)))

    result = open_meteo.fetch_current_weather("부산")
...
def test_unknown_city_is_rejected(monkeypatch) -> None:
    monkeypatch.setattr(open_meteo.httpx, "get", lambda *args, **kwargs: FakeResponse({"results": []}))

    try:
        open_meteo.geocode_city("없는도시")
